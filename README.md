...
* `:align-single-column-lines?` -
  true if cljfmt should include lines with single columns (where the value
  is on the next line) when calculating column alignment. When false, forms
  with wrapped values won't cause excessive 
  horizontal padding in other lines.
  Defaults to false. Only applies when `:align-form-columns?` or
  `:align-map-columns?` is true. **Experimental.**
